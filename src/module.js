...
class QuickFunctions {
     ...
     * Obtains the Image buffer from a querySelector("#photo");
     * @param {string} arrayBuffer 
     */
    static async buffToImg(arrayBuffer) {
    ...
    static async CreateDB(dbName) {
        ...
        var con = mysql.createConnection({
            host: this.dbConnect(host),
            user: this.dbConnect(user),
            password: this.dbConnect(password),
            database: dbName
        })
    ...
    static async TableCreate(tblName, extras) {
        ...
        if (Number(extras)) throw new Error("DataBase Content can't be a number!");
     ...
     */
    static async uFMention(mention) {
